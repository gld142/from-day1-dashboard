...
export interface AnimatedGradientProps {
    ...
    style?: CSSProperties;
    className?: string;
    /* ─── Ajouts Day 1 (usage en couche de fond) ─── */
    /** Rendu quand WebGL est indisponible ; `null` = rien (fond plat du parent). */
    fallback?: React.ReactNode;
    /** Suspend la boucle de rendu (hors écran, onglet caché…). */
    paused?: boolean;
}
...
export function AnimatedGradient({
    ...
    /* Day 1 : l'état « monté » d'origine était superflu (les effets ne tournent
       qu'après montage, les refs sont posées) et déclenchait
    ...
    useEffect(() => {
        ...
        /* Signalement différé : pas de setState synchrone dans le corps de
           l'effet (react-hooks/set-state-in-effect) ; le fallback se rend au
           tick suivant. */
        const failWebGL = () => {
        ...
        try {
            /* Day 1 : canvas opaque (alpha: false). Le shader n'émet pas de
               couleurs prémultipliées et un canvas WebGL transparent se
               compose mal (halo hors de la boîte en Chromium headless) ;
               la transparence se fait en CSS (opacity) sur le parent. */
    ...
    void main() {
      gl_Position = a_position;
            ...
            /* Temps écoulé hors pause : le dégradé reprend où il s'était arrêté. */
            ...
            const animate = (time: number) => {
                ...
                const elapsed = (time - startTimeRef.current - pausedTotal) / 1000;
    ...
    const resolvedFallback =
        fallback === undefined ? (
            <WebGLFallback className={cn("absolute inset-0 overflow-hidden", className)} />
        ) : (
            fallback
        );
