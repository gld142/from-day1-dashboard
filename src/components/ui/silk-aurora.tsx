...
export interface SilkAuroraProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** Voiles dégradés par-dessus le canvas (désactivés en couche de fond). */
  overlays?: boolean;
...
export function SilkAurora({
  ...
  React.useEffect(() => {
    ...
    /* Signalement différé : pas de setState synchrone dans le corps de l'effet
       (react-hooks/set-state-in-effect) ; le fallback se rend au tick suivant. */
    const failWebGL = () => {
    ...
    try {
      ...
      /* Temps écoulé hors pause : l'aurora reprend là où elle s'était arrêtée. */
      ...
      const render = (now: number) => {
        ...
        const elapsed = reducedMotion ? 8 : (now - start - pausedTotal) / 1000;
  ...
  const resolvedFallback = fallback === undefined ? fallbackContent : fallback;
